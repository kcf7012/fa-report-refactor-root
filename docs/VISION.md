# v3.0 願景:智慧化 FA 報告改善(已實現於 v3.0.1)

> **目標**:把「自動套樣板」升級為「智慧化評估 + 自動改善」
> **核心**:結合 LLM 推理、樣板系統、視覺設計原則,讓每次改善都是「量身打造」
> **狀態**:✅ v3.0 全部實現於 v3.0.0;v3.0.1 補充 .ppt 轉換、pre-commit、uv.lock;**v4.0 待規劃**
...
**v4.0 待規劃項目**(超出 v3.0 範圍):
- 圖像評估能力(GPT-4o-vision、Claude vision 等)
- 真正的主動學習(根據過往改善結果調整樣板)
- 多模態 LLM 整合(Anthropic、Ollama 本地模型)

詳見 `docs/10_api_reference.md` + 技能包 `CHANGELOG.md` v3.0.0 / v3.0.1 條目。
